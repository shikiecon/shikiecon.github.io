...
AES算法中的大部分操作都定义在Galois域$\mathrm{GF}(2^8)$上, 每个状态矩阵中的字节$\{b_7\,b_6\,b_5\,b_4\,b_3\,b_2\,b_1\,b_0\}$都可以表示为多项式
$$
...
- 轮常量异或: 将字节替换得到的列向量与轮常量$\mathrm{Rcon}[l]$进行异或, 这里的$l$​为轮数.[^6]
  ...
  [^6]: 轮常量是一个固定的向量, 具体可见[FIPS 197](https://doi.org/10.6028/NIST.FIPS.197-upd1).
...
\end{array} \tag{A.5}
$$
第一轮加密结束, 随后的第二轮至第九轮加密过程完全相同, 只是轮常量随轮数发生变化, 最后一轮除了不进行列混淆外, 其余步骤也与之前相同. 最后, 我们将十轮加密得到的状态矩阵展开如下, 最后一轮得到的状态矩阵即为AES-128最终得到的密文.
...
\begin{array}{|c|c|c|c|}
\hline
\mathrm{3b}&\mathrm{2e} &\mathrm{08} & \mathrm{e2}\\ \hline
\mathrm{8a}&\mathrm{57} &\mathrm{c1} & \mathrm{aa}\\ \hline
\mathrm{fa}&\mathrm{fe} &\mathrm{d4} &\mathrm{cb} \\ \hline
\mathrm{26}&\mathrm{67} &\mathrm{87} & \mathrm{ef}\\ \hline
\end{array} \,\,\oplus\,\,
\begin{array}{|c|c|c|c|}
\hline
\mathrm{29}&\mathrm{f8} &\mathrm{6e} & \mathrm{0e}\\ \hline
\mathrm{76}&\mathrm{e4} &\mathrm{5f} & \mathrm{09}\\ \hline
\mathrm{cf}&\mathrm{ad} &\mathrm{44} &\mathrm{db} \\ \hline
\mathrm{f2}&\mathrm{02} &\mathrm{ea} & \mathrm{e1}\\ \hline
\end{array}\,\,=\,\,
\begin{array}{|c|c|c|c|}
\hline
\mathrm{12}&\mathrm{d6} &\mathrm{66} & \mathrm{ec}\\ \hline
\mathrm{fc}&\mathrm{b3} &\mathrm{9e} & \mathrm{a3}\\ \hline
\mathrm{35}&\mathrm{53} &\mathrm{90} &\mathrm{10} \\ \hline
\mathrm{d4}&\mathrm{65} &\mathrm{6d} & \mathrm{0e}\\ \hline
\end{array} \tag{A.6}
$$
...
\hline
\mathrm{12}&\mathrm{d6} &\mathrm{66} & \mathrm{ec}\\ \hline
\mathrm{a3}&\mathrm{fc} &\mathrm{b3} & \mathrm{9e}\\ \hline
\mathrm{90}&\mathrm{10} &\mathrm{35} &\mathrm{53} \\ \hline
\mathrm{65}&\mathrm{6d} &\mathrm{0e} & \mathrm{d4}\\ \hline
\end{array} \tag{A.7}
$$
...
\begin{array}{|c|c|c|c|}
\hline
\mathrm{39}&\mathrm{4a} &\mathrm{d3} & \mathrm{83}\\ \hline
\mathrm{71}&\mathrm{55} &\mathrm{4b} & \mathrm{df}\\ \hline
\mathrm{96}&\mathrm{7c} &\mathrm{d9} &\mathrm{50} \\ \hline
\mathrm{bc}&\mathrm{b3} &\mathrm{d7} & \mathrm{19}\\ \hline
\end{array} \tag{A.8}
$$
...
\begin{array}{|c|c|c|c|}
\hline
\mathrm{ae}&\mathrm{d1} &\mathrm{96} & \mathrm{60}\\ \hline
\mathrm{ad}&\mathrm{92} &\mathrm{bb} & \mathrm{56}\\ \hline
\mathrm{e4}&\mathrm{62} &\mathrm{e9} &\mathrm{9f} \\ \hline
\mathrm{22}&\mathrm{f0} &\mathrm{e8} & \mathrm{0b}\\ \hline
\end{array}\,\,=\,\,
\begin{array}{|c|c|c|c|}
\hline
